...
module BlueRipple.Model.Evangelical.PRRIModel
  (
    module BlueRipple.Model.Evangelical.PRRIModel
  )
where
...
import qualified BlueRipple.Data.ACS_Tables as ACST
...
F.declareColumn "WhiteEvangelicalOrMormonPct" ''Double



whiteEvangelicalOrMormonByState :: (BRK.KnitEffects r, BRCC.CacheEffects r)
                                => K.Sem r (K.ActionWithCacheTime r (F.FrameRec [GT.StateFIPS, BR.Population, BR.PRRIWhiteEvangelicalPct, BR.PRRIMormonPct]))
whiteEvangelicalOrMormonByState = do
  prriByCounty_C <- BR.prriLoader
  BRCC.retrieveOrMakeFrame "data/PRRI/whiteEvangelicalOrMormonPctByState.bin" prriByCounty_C $ \prri -> do
    let pop r = r ^. BR.population
        wePct r = (realToFrac $ r ^. BR.pRRIWhiteEvangelicalPct) / 100
        mPct r = (realToFrac $ r ^. BR.pRRIMormonPct) / 100
        safeDiv x y = if y > 0 then x / y else 0
        popWgtd :: (a -> Int) -> (a -> Double) -> FL.Fold a Int
        popWgtd p f = (\x y -> round $ 100 * safeDiv x y) <$> (FL.premap (\r -> realToFrac (p r) * f r) FL.sum) <*> fmap realToFrac (FL.premap p FL.sum)
    let innerFld :: FL.Fold
                    (F.Record [BR.Population, BR.PRRIWhiteEvangelicalPct, BR.PRRIMormonPct])
                    (F.Record [BR.Population, BR.PRRIWhiteEvangelicalPct, BR.PRRIMormonPct])
        innerFld = (\ps pwWE pwM -> ps F.&: pwWE F.&: pwM F.&: V.RNil)
                   <$> FL.premap pop FL.sum
                   <*> popWgtd pop wePct
                   <*> popWgtd pop mPct
        fld = FMR.concatFold
              $ FMR.mapReduceFold
              FMR.noUnpack
              (FMR.assignKeysAndData @'[GT.StateFIPS] @[BR.Population, BR.PRRIWhiteEvangelicalPct, BR.PRRIMormonPct])
              (FMR.foldAndAddKey innerFld)
    pure $ FL.fold fld prri

whiteEvangelicalOrMormonBySLDViaState :: K.Sem r (K.ActionWithCacheTime r (F.FrameRec (ACST.LDLocationR V.++ [BR.PRRIWhiteEvangelicalPct, BR.PRRIMormonPct])))
whiteEvangelicalOrMormonBySLDViaState = undefined


{-
runPRRIModel :: forall l r ks b .
                (K.KnitEffects r
                , BRCC.CacheEffects r
                , V.RMap l
                , Ord (F.Record l)
...
runPRRIModel cesYear cacheStructure psType sp mc psData_C = do
...
data InputData = InputData { idPRRI :: F.FrameRec BR.PRRICols
                           , idCountyDemographics :: F.FrameRec ACST.LoadedCensusTablesByCounty
                           }

data ModelConfig = ModelConfig {}

data ModelData = ModelData { mdWhiteEvangelicalPct :: S.RealArrayE
                           , mdMormonPct :: S.RealArrayE
                           , mdDemoCounts :: S.Array S.IntArrayE
                           }

prriModelData ::  forall gq . ModelConfig -> S.StanModelBuilderEff InputData gq (ModelData CESByCD b)
prriModelData mc = do
